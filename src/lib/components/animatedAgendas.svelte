<script lang="ts">
	...
	export let randomAgendaMap: Map<number, string>;
	...
	{#each Array(numberOfRows) as _, i (i)}
		...
		>
			{#each Array(numberOfColumns) as _, j (j)}
				<div
					class={`animatedTextContainer ${i * numberOfColumns + j === 2 ? 'mainAgenda' : ''}`}
					style={i * numberOfColumns + j === 2
						? ''
						: `padding: ${Math.max(0.5, Math.random() * 1.5)}%;`}
				>
					{#if randomAgendaMap.has(i * numberOfColumns + j)}
						<p>
							{`${sentences[i * numberOfColumns + j]}${
								sentences[i * numberOfColumns + j].length ===
									randomAgendaMap.get(i * numberOfColumns + j)?.length ||
